...
public class bl_HUDText : MonoBehaviour {
        ...
        if (Event.current.type == EventType.Repaint)
        {
            for (int i = 0; i < texts.Count; i++)
            {
                ...
                else//if UI visible
                {
                    ...
                    Collider c= texts[i].m_Transform.GetComponent<Collider>();
                    //Get center up of target
                    Vector3 position = c.bounds.center + (((Vector3.up * c.bounds.size.y) * 0.5f));
    ...
    private int ScreenPosition(Transform t)
    {
        ...
        int p = (int)(m_Cam.WorldToScreenPoint(c.bounds.center + (((Vector3.up * c.bounds.size.y) * 0.5f))).y - this.m_Cam.WorldToScreenPoint(c.bounds.center - (((Vector3.up * c.bounds.size.y) * 0.5f))).y);
        return p;
    }
}