    ...
    where P : AbstractEntity
{
    ...
    {
        string str2 = string.Format ("{0}{1}{2}", UnityEngine.Application.dataPath, @"\Script\LocalData\",FileName ); 
        using (GameDataTableParser parse = new GameDataTableParser(str2))
        {
            while (!parse.Eof)
            {
                //创建实体
