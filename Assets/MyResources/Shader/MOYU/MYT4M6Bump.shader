Shader "MOYU/T4M6Bump" 
{
	...
	{ 
		...
		_BumpMap4("Bump4 (RGB)", 2D) = "black" {}
		_BunpZ4("法线深度调节", Range(0.1, 4.0)) = 0.1

		_Splat5("Layer 5 (B)", 2D) = "white" {}
		_BumpMap5("Bump5 (RGB)", 2D) = "black" {}
		_BunpZ5("法线深度调节", Range(0.1, 4.0)) = 0.1
	...
	SubShader 
	{
		...
		{ 
			...
			uniform sampler2D _BumpMap0, _BumpMap1, _BumpMap2, _BumpMap3, _BumpMap4, _BumpMap5;
			uniform half _BunpZ0, _BunpZ1, _BunpZ2, _BunpZ3, _BunpZ4, _BunpZ5;

			fixed3 CalNormal(sampler2D bump, half2 uv, half BunpZ,out fixed alpha)
			{
			...
			fixed4 frag (v2f_vertex i) : COLOR
			{
				...
				fixed alpha0, alpha1, alpha2, alpha3, alpha4, alpha5;
				fixed3 sp1B = CalNormal(_BumpMap0, i.uv1.xy, _BunpZ0, alpha0);
				fixed3 sp2B = CalNormal(_BumpMap1, i.uv1.zw, _BunpZ1, alpha1);
				fixed3 sp3B = CalNormal(_BumpMap2, i.uv2.xy, _BunpZ2, alpha2);

				fixed3 sp4B = CalNormal(_BumpMap3, i.uv2.zw, _BunpZ3, alpha3);
				fixed3 sp5B = CalNormal(_BumpMap4, i.uv3.xy, _BunpZ4, alpha4);
				fixed3 sp6B = CalNormal(_BumpMap5, i.uv3.zw, _BunpZ5, alpha5);

				fixed3 Normal = sp1B * splat_control.r + sp2B * splat_control.g + sp3B * splat_control.b;
				fixed3 Normal2 = sp4B * splat_control2.r + sp5B * splat_control2.g + sp6B * splat_control2.b;
				Normal += splat_control.a * Normal2;
				o.Normal = normalize(Normal);

				fixed alpha = alpha0 * splat_control.r + alpha1 * splat_control.g + alpha2 * splat_control.b;
				fixed alpha_ = alpha3 * splat_control2.r + alpha4 * splat_control2.g + alpha5 * splat_control2.b;
				alpha += splat_control.a * alpha_;
