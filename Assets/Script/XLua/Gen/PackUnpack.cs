...
namespace XLua
{
    public static partial class CopyByValue
    {
		...
		public static void UnPack(ObjectTranslator translator, RealStatePtr L, int idx, out CClientRoleData val)
		{
		    val = new CClientRoleData();
            int top = LuaAPI.lua_gettop(L);
			
			if (Utils.LoadField(L, idx, "RefID"))
            {
			    
                translator.Get(L, top + 1, out val.RefID);
			...
			if (Utils.LoadField(L, idx, "SN"))
            {
			    
                translator.Get(L, top + 1, out val.SN);
				
            }
            LuaAPI.lua_pop(L, 1);
			
			if (Utils.LoadField(L, idx, "Dir_X"))
            {
			    
                translator.Get(L, top + 1, out val.Dir_X);
				
            }
            LuaAPI.lua_pop(L, 1);
			
			if (Utils.LoadField(L, idx, "Dir_Z"))
            {
			    
                translator.Get(L, top + 1, out val.Dir_Z);
        ...
        public static bool Pack(IntPtr buff, int offset, CClientRoleData field)
        {
            
            if(!Pack(buff, offset, field.RefID))
            {
                return false;
            }
            
            if(!Pack(buff, offset + 4, field.X))
            {
                return false;
            }
            
            if(!Pack(buff, offset + 8, field.Y))
            {
                return false;
            }
            
            if(!Pack(buff, offset + 12, field.SN))
            {
                return false;
            }
            
            if(!Pack(buff, offset + 16, field.Dir_X))
            {
                return false;
            }
            
            if(!Pack(buff, offset + 20, field.Dir_Z))
            {
                return false;
            }
        ...
        public static bool UnPack(IntPtr buff, int offset, out CClientRoleData field)
        {
            field = default(CClientRoleData);
            
            if(!UnPack(buff, offset, out field.RefID))
            {
                return false;
            }
            
            if(!UnPack(buff, offset + 4, out field.X))
            {
                return false;
            }
            
            if(!UnPack(buff, offset + 8, out field.Y))
            {
                return false;
            }
            
            if(!UnPack(buff, offset + 12, out field.SN))
            {
                return false;
            }
            
            if(!UnPack(buff, offset + 16, out field.Dir_X))
            {
                return false;
            }
            
            if(!UnPack(buff, offset + 20, out field.Dir_Z))
            {
                return false;
            }
